...
const ACHIEVEMENTS = [
  { id: 'hand', name: '初识手语', description: '完成第一次手语识别', icon: 'hand', sortOrder: 1 },
  { id: 'message_circle', name: '交流达人', description: '在社区发布10条动态', icon: 'message_circle', sortOrder: 2 },
  { id: 'target', name: '坚持不懈', description: '连续登录7天', icon: 'target', sortOrder: 3 },
  { id: 'volume2', name: '聆听者', description: '使用声音检测功能50次', icon: 'volume2', sortOrder: 4 },
  { id: 'star', name: '社区明星', description: '获得100个赞', icon: 'star', sortOrder: 5 },
  { id: 'trophy', name: '手语大师', description: '完成所有基础课程', icon: 'trophy', sortOrder: 6 },
]

app.get("/make-server-481f4acb/achievements", async (c) => {
  try {
    const user = await authenticateUser(c)
    if (!user) return c.json({ error: '请先登录' }, 401)

    const userAchievementsStr = await safeKv.get(`achievements_${user.id}`)
    const userAchievements: Record<string, { unlockedAt: number, progress: number }> = userAchievementsStr
      ? JSON.parse(String(userAchievementsStr))
      : {}

    const statsStr = await safeKv.get(`user_stats_${user.id}`)
    const stats = statsStr ? (typeof statsStr === 'string' ? JSON.parse(statsStr) : statsStr) : null

    const result = ACHIEVEMENTS.map((a) => {
      const ua = userAchievements[a.id]
      let unlocked = !!ua
      // auto-unlock based on stats
      if (!unlocked && stats) {
        if (a.id === 'hand' && stats.totalTranslations >= 1) unlocked = true
        if (a.id === 'target' && stats.loginStreak >= 7) unlocked = true
        if (a.id === 'volume2' && stats.totalSoundDetections >= 50) unlocked = true
      }
      return {
        id: a.id,
        name: a.name,
        ...
        unlockedAt: (ua?.unlockedAt || unlocked ? new Date(ua?.unlockedAt || 0).toISOString() : null),
        progress: ua?.progress || (unlocked ? 100 : 0),
      }
    })

    return c.json(result)
  } catch (error: any) {
    console.error('[成就] 异常:', error)
    return c.json({ error: error.message }, 500)
  }
})
...
app.get("/make-server-481f4acb/user/:id/followers/count", async (c) => {
  try {
    ...
    const followersStr = await safeKv.get(`followers_${userId}`)
    const followers: string[] = followersStr ? JSON.parse(String(followersStr)) : []
    return c.json({ count: followers.length })
  } catch (error: any) {
...
app.get("/make-server-481f4acb/user/:id/following/count", async (c) => {
  try {
    const userId = c.req.param('id')
    const followingStr = await safeKv.get(`following_${userId}`)
    const following: string[] = followingStr ? JSON.parse(String(followingStr)) : []
    return c.json({ count: following.length })
  } catch (error: any) {
...
app.post("/make-server-481f4acb/user/:id/follow", async (c) => {
  try {
    ...
    const followingStr = await safeKv.get(`following_${user.id}`)
    const following: string[] = followingStr ? JSON.parse(String(followingStr)) : []
    if (!following.includes(targetId)) {
      following.push(targetId)
      await safeKv.set(`following_${user.id}`, JSON.stringify(following))
    }

    // add to target's followers list
    const followersStr = await safeKv.get(`followers_${targetId}`)
    const followers: string[] = followersStr ? JSON.parse(String(followersStr)) : []
    if (!followers.includes(user.id)) {
      followers.push(user.id)
      await safeKv.set(`followers_${targetId}`, JSON.stringify(followers))
    }
...
app.delete("/make-server-481f4acb/user/:id/follow", async (c) => {
  try {
    ...
    const followersStr = await safeKv.get(`followers_${targetId}`)
    const followers: string[] = followersStr ? JSON.parse(String(followersStr)) : []
    const fidx = followers.indexOf(user.id)
    if (fidx !== -1) {
...
app.get("/make-server-481f4acb/user/:id/is-following", async (c) => {
  try {
    ...
    const followingStr = await safeKv.get(`following_${user.id}`)
    const following: string[] = followingStr ? JSON.parse(String(followingStr)) : []
    return c.json({ following: following.includes(targetId) })
  } catch (error: any) {
    console.error('[检查关注] 异常:', error)
    return c.json({ error: error.message }, 500)
  }
})
...
app.get("/make-server-481f4acb/user/settings", async (c) => {
  try {
    ...
    const settingsStr = await safeKv.get(`settings_${user.id}`)
    const defaults = { notification: true, vibration: true, language: 'zh-CN' }
    const settings = settingsStr ? { ...defaults, ...JSON.parse(String(settingsStr)) } : defaults
    return c.json(settings)
  } catch (error: any) {
    console.error('[获取设置] 异常:', error)
    return c.json({ error: error.message }, 500)
  }
})
...
app.put("/make-server-481f4acb/user/settings", async (c) => {
  try {
    ...
    const body = await c.req.json()
    const defaults = { notification: true, vibration: true, language: 'zh-CN' }
    const existingStr = await safeKv.get(`settings_${user.id}`)
    const existing = existingStr ? JSON.parse(String(existingStr)) : {}
    const merged = { ...defaults, ...existing, ...body }
    await safeKv.set(`settings_${user.id}`, JSON.stringify(merged))
    return c.json(merged)
  } catch (error: any) {
    console.error('[更新设置] 异常:', error)
    return c.json({ error: error.message }, 500)
  }
})

// ============================================================
// 13. 积分余额（独立于 /user/points 的明细接口）
// ============================================================

app.get("/make-server-481f4acb/points", async (c) => {
  try {
    const user = await authenticateUser(c)
    if (!user) return c.json({ error: '请先登录' }, 401)

    const statsStr = await safeKv.get(`user_stats_${user.id}`)
    const stats = statsStr ? (typeof statsStr === 'string' ? JSON.parse(statsStr) : statsStr) : { points: 0 }
    ...
    if (Array.isArray(records)) {
      for (const val of records) {
        try {
          ...
          if (r.points > 0 || r.amount > 0) {
            totalEarned += (r.points || r.amount || 0)
          }
        } catch (_) {}
      }
    }

    return c.json({ balance: stats.points || 0, totalEarned })
  } catch (error: any) {
    console.error('[积分余额] 异常:', error)
    return c.json({ error: error.message }, 500)
  }
})

app.get("/make-server-481f4acb/points/history", async (c) => {
  try {
    const user = await authenticateUser(c)
    if (!user) return c.json({ error: '请先登录' }, 401)
    ...
    const all = Array.isArray(records)
      ? records
          .map((val: any) => {
            const r = typeof val === 'string' ? JSON.parse(val) : val
            return {
              id: r.id || `${user.id}_${Date.now()}`,
              amount: r.points || r.amount || 0,
              reason: r.title || r.reason || '',
              createdAt: new Date(r.createdAt || r.date || 0).toISOString(),
            }
          })
          .sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
      : []

    return c.json({ records: all, total: all.length, limit: all.length, offset: 0 })
  } catch (error: any) {
    console.error('[积分历史] 异常:', error)
    return c.json({ error: error.message }, 500)
  }
})
